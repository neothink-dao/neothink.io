import { Scenario, SimUser, SimContext, ScenarioResult } from '../scenarioTypes';
import * as queries from '../queries';

function pickTokenForPersona(persona: string, site: string): 'LUCK' | 'LIVE' | 'LOVE' | 'LIFE' {
  // Persona + site-specific logic (can be expanded)
  if (site === 'hub') return persona === 'ascender' ? 'LUCK' : 'LIVE';
  if (site === 'ascenders') return persona === 'immortal' ? 'LIFE' : 'LUCK';
  if (site === 'neothinkers') return persona === 'neothinker' ? 'LOVE' : 'LIVE';
  if (site === 'immortals') return persona === 'immortal' ? 'LIFE' : 'LUCK';
  return 'LUCK';
}
...
export const onboardingScenario: Scenario = {
  ...
  async run(users: SimUser[], context: SimContext): Promise<ScenarioResult> {
    ...
    for (const user of users) {
      ...
      for (const phase of this.phases) {
        ...
        // Earn event
        await queries.insertGamificationEvent(
          dbClient,
          user.id,
          persona,
          site,
          `${phase}_reward`,
          tokenType,
          user.tokenBalance && user.tokenBalance[tokenType] > diminishingThreshold ? baseReward / 2 : baseReward,
          { phase },
          simulationRunId
        );
        events.push({
