...
async function main() {
  ...
  const context: SimContext = {
    ...
    supabaseKey: SUPABASE_SERVICE_ROLE_KEY as string,
    dbClient,
    now: new Date(),
  };
  const result: ScenarioResult = await scenario.run(users, context);
  ...
  fs.writeFileSync(
    path.join(resultsDir, `sim-result-${ts}.json`),
    JSON.stringify({ config: simConfig, metrics: result.metrics, events: result.events }, null, 2)
  );
