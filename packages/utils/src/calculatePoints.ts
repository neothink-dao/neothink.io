...
interface CalculatePointsArgs {
  ...
  basePoints: number; // Base points for the specific action before multipliers
  streak: number; // User's current streak
  isTrialUser?: boolean;
  isInactive?: boolean; // Added to disable multipliers/bonuses
  // Add other potential factors like event multipliers if needed
}
...
export const calculatePoints = ({
  ...
  isTrialUser = false,
  isInactive = false,
}: CalculatePointsArgs): CalculatedPointsResult => {
