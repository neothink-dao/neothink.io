...
import { hasPlatformAccess, grantPlatformAccess, revokePlatformAccess, type PlatformSlug } from '@neothink/core';
...
export function PlatformAccess({ userId, onUpdate }: PlatformAccessProps) {
  const [platformAccess, setPlatformAccess] = useState<Record<PlatformSlug, boolean>>({
    hub: false,
    ascenders: false,
    neothinkers: false,
    immortals: false
  ...
  const [updating, setUpdating] = useState<PlatformSlug | null>(null);
  ...
  useEffect(() => {
    async function fetchAccess() {
      setLoading(true);
      try {
        const platforms: PlatformSlug[] = ['hub', 'ascenders', 'neothinkers', 'immortals'];
        const access: Record<PlatformSlug, boolean> = {
          hub: false,
          ascenders: false,
          neothinkers: false,
          immortals: false
  ...
  async function toggleAccess(platform: PlatformSlug) {
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="space-y-4">
            {(Object.keys(platformAccess) as PlatformSlug[]).map(platform => (
              <div key={platform} className="flex items-center justify-between">
                ...
                <Button
                  variant={platformAccess[platform] ? "destructive" : "default"}
                  size="sm"
                  onClick={() => toggleAccess(platform)}
