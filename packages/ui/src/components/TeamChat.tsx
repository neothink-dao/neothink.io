...
interface TeamChatProps {
  ...
  onSendMessage: (messageText: string) => void; // Function to handle sending message
  className?: string;
}
...
const TeamChat: React.FC<TeamChatProps> = ({
  ...
  return (
    <Card className={cn('flex flex-col h-[500px]', className)} style={{ backgroundColor: chatBoxColor }}>
              ...
              >
                ...
                >
                  ...
                  <p className="text-sm">{msg.text}</p>
                   ...
                   {msg.pointsAwarded && msg.platform && (
                     <div className="flex items-center justify-end mt-1">
                       <PointsCircle
                         points={`+${msg.pointsAwarded}`}
                         platformColor={platformColors[msg.platform] || platformColors.default}
                         size={20}
                       />
                     </div>
                   )}
      ...
      <CardFooter className="p-4 border-t border-zinc-700">
        <div className="flex w-full items-center space-x-2">
          <Input
            ...
            className="flex-1 bg-zinc-700 border-zinc-600 text-zinc-100 placeholder:text-zinc-400 focus:ring-blue-500"
          />
          <Button
            type="button"
            onClick={handleSend}
