...
const QuestCard = ({ title, description, points, actionText, platform, questType, isTrialUser = false, onActionClick, className, }) => {
    ...
    return (<Card className={cn('overflow-hidden', sizeClass, className)} style={{ borderColor: color }}>
      ...
      <CardFooter className="flex justify-between items-center p-4 bg-muted/50">
        ...
        <Button variant="outline" size="sm" onClick={onActionClick} disabled={isTrialUser && questType !== 'read'} // Allow only read action for trial users? Adjust as needed.
     style={{ borderColor: color, color: color }} className="hover:bg-accent hover:text-accent-foreground">
          {actionText}
        </Button>
      </CardFooter>
    </Card>);
