...
const StreakVortex = ({ streak, platform, className, }) => {
    ...
    useEffect(() => {
        ...
        const boost = Math.min(61.8 + (streak > 0 ? ((streak - 1) / (maxStreakForFullBoost - 1)) * 100 : 0), 161.8);
        setBoostPercentage(streak > 0 ? boost : 0);
    }, [streak, maxStreakForFullBoost]);
    return (<div className={cn('relative w-16 h-16', className)}>
      {/* Base Circle */}
      <div className="absolute inset-0 rounded-full border-2" style={{ borderColor: baseColor }}/>

      {/* Animated Fill Vortex */}
      <motion.div className="absolute inset-0 rounded-full overflow-hidden" style={{ transform: 'rotate(-90deg)' }} // Start from top
    >
        <motion.div className="absolute top-0 left-0 w-full h-full rounded-full" style={{
         ...
         <motion.div className="absolute inset-1 rounded-full border-t-2 border-b-2" style={{ borderColor: `${fillColor}80`, borderWidth: '1px' }} animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }}/>
      </motion.div>
...
export { StreakVortex };
//# sourceMappingURL=StreakVortex.js.map