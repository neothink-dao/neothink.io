...
export const ChatInterface = ({ appName, userId, onSendMessage, className = '' }) => {
    ...
    return (<div className={`flex flex-col h-full rounded-lg overflow-hidden shadow-xl ${className}`}>
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
        <AnimatePresence>
          {messages.map((message) => (<motion.div key={message.id} initial="hidden" animate="visible" exit="hidden" variants={messageVariants} transition={{ duration: 0.3 }} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
        ...
        {isTyping && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start">
      ...
      <form onSubmit={handleSubmit} className="p-4 bg-white border-t">
        <div className="flex space-x-2">
          <input type="text" value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} placeholder="Type your message..." className="flex-1 rounded-lg border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-opacity-50" style={{ '--tw-ring-color': `var(--${appName}-primary)` }}/>
          <button type="submit" disabled={!inputMessage.trim() || isTyping} className={`${theme.primary} text-white px-6 py-2 rounded-lg font-medium 
