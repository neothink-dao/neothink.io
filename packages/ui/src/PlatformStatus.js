 ...
 */
export function PlatformStatus({ platform = 'all', showDetails = false, className = '' }) {
    ...
    return (<div className={className}>
      ...
      <div className="flex items-center space-x-2">
        <div className={`h-3 w-3 rounded-full ${getStatusColor(primaryStatus.status)}`} title={primaryStatus.title}></div>
      ...
      {showDetails && primaryStatus.message && (<div className="mt-2 text-sm text-gray-600 border-l-4 pl-3 border-gray-300">
          {primaryStatus.message}
        </div>)}

      {/* Show all platform statuses when viewing 'all' */}
      {showDetails && platform === 'all' && (<div className="mt-4 space-y-2">
          {statuses
                .filter(s => s.platform !== 'all')
                .map((status) => (<div key={status.platform} className="flex items-center space-x-2">
      ...
      {/* Last updated time */}
      {showDetails && (<div className="mt-3 text-xs text-gray-400">
