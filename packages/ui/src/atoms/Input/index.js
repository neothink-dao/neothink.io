...
export const Input = forwardRef((_a, ref) => {
    var { className, inputSize, size, state, leftElement, rightElement, error, success } = _a, props = __rest(_a, ["className", "inputSize", "size", "state", "leftElement", "rightElement", "error", "success"]);
    ...
    const sizeToUse = inputSize || size;
    return (<div className="w-full">
        <div className="relative">
          {leftElement && (<div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              {leftElement}
            </div>)}
          <input className={cn(inputVariants({ inputSize: sizeToUse, state: validationState }), leftElement && 'pl-10', rightElement && 'pr-10', className)} ref={ref} {...props}/>
          {rightElement && (<div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
              {rightElement}
            </div>)}
        </div>
