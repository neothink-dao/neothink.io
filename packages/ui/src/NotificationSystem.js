...
export const NotificationSystem = ({ userId, appName, maxNotifications = 5, autoHideDuration = 5000, position = 'top-right', className = '', }) => {
    ...
    const getNotificationIcon = (type) => {
        switch (type) {
            ...
            case 'error':
                return (<svg className="w-6 h-6 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            default:
                return (<svg className="w-6 h-6 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
          </svg>);
        }
    };
    return (<div className={`fixed z-50 flex flex-col ${positionStyles[position]} space-y-2 ${className}`}>
      <AnimatePresence>
        {visibleNotifications.map(notification => (<motion.div key={notification.id} initial={{ opacity: 0, y: position.includes('top') ? -20 : 20, x: 0 }} animate={{ opacity: 1, y: 0, x: 0 }} exit={{ opacity: 0, x: position.includes('right') ? 100 : -100 }} transition={{ duration: 0.2 }} className="bg-white rounded-lg shadow-lg overflow-hidden max-w-sm w-full" onClick={() => handleNotificationClick(notification)}>
...
export function useNotifications(userId, appName) {
    const [notifications, setNotifications] = useState([]);
