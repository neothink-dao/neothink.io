 ...
 */
export async function checkRateLimit(supabase, config) {
    const now = new Date();
    ...
    return {
        ...
        resetTime: new Date(new Date(existingData.window_start).getTime() + config.windowSeconds * 1000),
    };
}
/**
 * Helper function to create a rate limit config for IP-based rate limiting
 */
export function createIpRateLimit(ip, maxRequests = 100, windowSeconds = 60) {
    return {
        identifier: `ip:${ip}`,
        maxRequests,
        windowSeconds,
 ...
 */
export function createUserRateLimit(userId, maxRequests = 1000, windowSeconds = 60) {
    return {
