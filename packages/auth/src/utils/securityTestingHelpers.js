 ...
 */
export async function getRecentSecurityLogs(platformSlug = 'hub', limit = 100) {
 ...
 */
export async function getRateLimitRecords(identifier, platformSlug = 'hub') {
    const supabase = createPlatformClient(platformSlug);
    const { data, error } = await supabase
        .from('rate_limits')
        .select('*')
 ...
 */
export async function generateTestSecurityLogs(count = 10, platformSlug = 'hub') {
 ...
 */
export async function clearTestSecurityLogs(platformSlug = 'hub') {
    if (process.env.NODE_ENV === 'production') {
...
//# sourceMappingURL=securityTestingHelpers.js.map