...
import { VectorDocument, VectorSearchRequest, VectorSearchResponse, VectorSearchResult } from '../types';
 ...
 * Default options
 */
const DEFAULT_OPTIONS: SupabaseVectorStoreOptions = {
...
export class SupabaseVectorStore {
  private supabase;
  private options: SupabaseVectorStoreOptions;
  ...
  constructor(
    supabaseUrl: string,
    supabaseKey: string,
    options: SupabaseVectorStoreOptions = DEFAULT_OPTIONS
  ) {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      const hasVectorExtension = extensions.some((ext: any) => ext.name === 'vector');
   ...
   */
  async storeDocuments(documents: VectorDocument[]): Promise<number> {
    try {
   ...
   */
  async deleteDocumentsByPlatform(platform: PlatformSlug): Promise<number> {
    try {
      const { data, error } = await this.supabase
        .from(this.options.tableName!)
        .delete()
   ...
   * @returns Success status
   */
  async updateDocumentMetadata(id: string, metadata: Record<string, unknown>): Promise<boolean> {
    try {
      const { error } = await this.supabase
        .from(this.options.tableName!)
        .update({ metadata })
