 ...
 */
export async function executePagedQuery<T extends keyof Database['public']['Tables']>(
  client: SupabaseClient<Database>,
  table: T,
  params: QueryParams = {}
): Promise<PaginatedResponse<Database['public']['Tables'][T]['Row']>> {
  // Default pagination values
  const page = params.page || 1;
  const limit = params.limit || 20;
  
  // Build the query with pagination
  const paginatedQuery = buildQuery(client, table, {
    ...params,
    page,
    limit,
  });

  // Execute query - fix: count is now part of response
  const { data, error, count } = await paginatedQuery.select();
  ...
  return {
    data: data as Array<Database['public']['Tables'][T]['Row']> || [],
    total: totalCount,
    page,
    limit,
    hasMore,
 ...
 */
export function arrayContains(field: string, value: string | string[]): Record<string, any> {
  if (Array.isArray(value)) {
 ...
 * @returns Properly structured filter object
 */
export function textSearch(field: string, searchTerm: string): Record<string, any> {
 ...
 * @returns Properly structured filter object
 */
export function rangeFilter(field: string, min?: number | string, max?: number | string): Record<string, any> {
 ...
 * @returns Properly structured filter object
 */
export function dateRangeFilter(field: string, startDate?: string, endDate?: string): Record<string, any> {
