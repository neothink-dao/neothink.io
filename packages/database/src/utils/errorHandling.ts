 ...
 */
export function handleDatabaseError(error: PostgrestError | null): ApiError {
  if (!error) {
    return {
 ...
 */
export async function withErrorHandling<T>(databaseFn: () => Promise<T>): Promise<T> {
  try {
 ...
 */
function cleanErrorDetails(details: any): Record<string, any> | undefined {
