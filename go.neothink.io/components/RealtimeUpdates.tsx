 ...
 */
export function RealtimeUpdates({ platform }: { platform: string }) {
  ...
  useEffect(() => {
    ...
    try {
      const channel = supabase
        .channel(`content-updates-${platform}`, {
          config: {
        ...
        .on('postgres_changes', {
          ...
          schema: 'public',
          table: 'content',
          filter: `platform=eq.${platform}` // Platform-specific filter
        }, (payload) => {
          // Measure latency
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4">
      ...
      {error && (
        <div className="mb-4 p-2 text-sm bg-red-50 text-red-600 rounded border border-red-200">
          {error}
      ...
      {updates.length === 0 ? (
        <p className="text-sm text-gray-500 italic">
          No updates yet. Changes to content will appear here in real-time.
        </p>
      ) : (
            ...
            >
              <div className="flex justify-between mb-1">
                <span className={`font-medium ${
                  update.eventType === 'INSERT' ? 'text-green-600' : 
              ...
              <p className="text-xs text-gray-600">
                {update.eventType === 'DELETE' 
                  ? `ID: ${update.old.id}` 
                  : `"${update.new.title}" (ID: ${update.new.id})`}
              </p>
            </div>
          ))}
        </div>
      ...
      <div className="mt-4 text-center text-xs text-gray-500">
        Using Supabase Realtime Broadcast for live updates
