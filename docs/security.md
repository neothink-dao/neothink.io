# Neothink Security Guide & Checklist
...
- [x] **JWT-based Authentication**: Secure token-based authentication with proper expiration and refresh mechanisms
- [x] **PKCE Flow**: Enhanced PKCE (Proof Key for Code Exchange) OAuth flow for secure authorization
- [x] **Row Level Security (RLS)**: Database-level security policies to restrict data access
- [x] **Role-Based Access Control (RBAC)**: Fine-grained permission system based on user roles
...
- [x] **Rate Limiting**: API rate limiting to prevent brute force attacks
...
```sql
-- Example policy from supabase/migrations
...
```typescript
// lib/middleware/unified-middleware.ts
...
export function useSecureForm<T extends z.ZodType<any, any>>(schema: T) {
  ...
  const validate = (formData: unknown): boolean => {
    ...
    if (!result.success) {
      // Format Zod errors into a more usable shape
