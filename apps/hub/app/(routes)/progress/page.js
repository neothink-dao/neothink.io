...
const GamificationStatsPlaceholder = ({ stats }) => {
    if (!stats)
        return <p>Loading gamification stats...</p>;
    return (<div className="bg-zinc-100 p-4 rounded-lg shadow">
...
export default async function ProgressPage() {
    ...
    return (<div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <GamificationStats stats={gamificationStats} isLoading={!gamificationStats && !gamificationError} error={gamificationError === null || gamificationError === void 0 ? void 0 : gamificationError.message}/>
      </div>

      <div className="mb-8">
        <h2 className="text-2xl font-semibold mb-4">Token Balances</h2>
        <TokenBalance userId={session.user.id} supabaseUrl={process.env.NEXT_PUBLIC_SUPABASE_URL} supabaseKey={process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY}/>
      </div>

      <div>
        <h2 className="text-2xl font-semibold mb-4">Token History</h2>
        <TokenHistory userId={session.user.id} supabaseUrl={process.env.NEXT_PUBLIC_SUPABASE_URL} supabaseKey={process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY}/>
      </div>
    </div>);
}
