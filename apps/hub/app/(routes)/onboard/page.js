...
function OnboardingSteps() {
    ...
    return (<div className="divide-y divide-gray-100">
      {steps.map(step => (<Link key={step.id} href={step.link} className="block p-6 hover:bg-gray-50 transition-colors">
          <div className="flex items-start">
            <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center mr-4 mt-0.5 ${step.completed ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
              {step.completed ? (<CheckCircle size={20}/>) : (<span className="text-sm font-medium">{step.id}</span>)}
            </div>
