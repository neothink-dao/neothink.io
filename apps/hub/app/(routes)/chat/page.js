...
import { Card, CardContent, CardHeader, CardFooter, CardTitle, CardDescription, Button, Textarea, ScrollArea } from '@neothink/ui';
...
export default function ChatPage() {
    ...
    const updateRoomTheme = (room) => {
        const themeKey = Object.keys(ROOM_THEMES).find(key => ROOM_THEMES[key].name === room.name);
        if (themeKey) {
    ...
    const isOwnMessage = (senderId) => {
        return (profile === null || profile === void 0 ? void 0 : profile.id) === senderId;
    };
    ...
    const isActiveRoom = (roomId) => {
        return (currentRoom === null || currentRoom === void 0 ? void 0 : currentRoom.id) === roomId;
    };
    // Get next meeting time for themed rooms
    const getNextMeetingTime = (theme) => {
        if (!theme.meetingDay || !theme.meetingTime)
            return null;
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 md:col-span-4 lg:col-span-3">
          <Card className="h-full">
            ...
            return (<div key={room.id} className={`px-4 py-3 rounded-lg cursor-pointer transition-all
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="flex-grow">
                          <div className="flex items-center">
                            ...
                            {!room.is_accessible && (<LockIcon className="h-3.5 w-3.5 ml-2 text-gray-400"/>)}
                          </div>
        ...
        <div className="col-span-12 md:col-span-8 lg:col-span-9">
          {currentRoom ? (<Card className="h-full flex flex-col">
              {/* Chat Header */}
              <div className={`bg-gradient-to-r ${roomTheme.color} text-white px-6 py-4 rounded-t-lg`}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {roomTheme.meetingDay && (<Badge variant="secondary" className="bg-white/20 text-white border-none">
                ...
                {roomTheme.meetingDay && (<div className="mt-3 text-xs bg-white/10 p-2 rounded flex items-center">
              ...
              <CardContent className="flex-grow p-0 overflow-hidden">
                <ScrollArea className="h-[500px] p-6">
                  {messagesLoading ? (<div className="space-y-4">
                      {[1, 2, 3].map(i => (<div key={i} className="flex items-start gap-2">
                          <Skeleton className="h-10 w-10 rounded-full"/>
                          <div className="space-y-2">
                    ...
                    </div>) : messages.length === 0 ? (<div className="h-full flex flex-col items-center justify-center text-center p-6">
                    ...
                    return (<div key={message.id} className={`flex items-start gap-3 ${isOwn ? 'flex-row-reverse' : ''}`}>
                            <div className={`max-w-[70%] ${isOwn ? 'text-right' : ''}`}>
                              <div className="flex items-center gap-2 mb-1 text-sm">
                                <span className={`font-medium ${isOwn ? 'ml-auto' : ''}`}>
                                  {((_a = message.sender) === null || _a === void 0 ? void 0 : _a.full_name) || 'Neothink Member'}
                                </span>
                                <span className="text-xs text-muted-foreground">
                                  {formatMessageTime(message.created_at)}
                                </span>
                                {message.token_tag && (<Badge variant="outline" className={`${ROOM_THEMES[message.token_tag].textColor} text-xs`}>
                                    {message.token_tag}
                                  </Badge>)}
                              </div>
              ...
              <CardFooter className="p-4 border-t">
                ...
                sendMessage();
                  ...
                  <Textarea placeholder={`Type your message in ${currentRoom.name}...`} value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} className="flex-grow min-h-[50px] max-h-[150px] resize-none"/>
                  <Button type="submit" className={`bg-gradient-to-r ${roomTheme.color} text-white px-6`} disabled={!inputMessage.trim()}>
                    <SendIcon className="h-5 w-5"/>
                  </Button>
                </form>
              </CardFooter>
            </Card>) : (<Card className="h-full flex items-center justify-center p-12">
