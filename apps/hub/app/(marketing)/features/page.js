    ...
    return (<div className="container py-10">
      ...
      <section className="space-y-6 py-8">
        <div className="flex max-w-[64rem] flex-col gap-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {coreFeatures.map((feature) => (<div key={feature.title} className="rounded-lg border p-6">
                <h3 className="font-bold">{feature.title}</h3>
                <p className="mt-2 text-sm text-muted-foreground">{feature.description}</p>
                <ul className="mt-4 space-y-2">
                  {feature.capabilities.map((capability) => (<li key={capability} className="text-sm">
                      • {capability}
                    </li>))}
                </ul>
              </div>))}
      ...
      <section className="space-y-6 py-8">
        <div className="flex max-w-[64rem] flex-col gap-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {integrations.map((integration) => (<div key={integration.title} className="rounded-lg border p-6">
      ...
      <section className="space-y-6 py-8">
        <div className="flex max-w-[64rem] flex-col gap-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {upcomingFeatures.map((feature) => (<div key={feature.title} className="rounded-lg border p-6">
