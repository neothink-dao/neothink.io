...
export default function Leaderboard() {
    ...
    useEffect(() => {
        ...
        async function fetchLeaders() {
            ...
            const xpMap = {};
            xpRows === null || xpRows === void 0 ? void 0 : xpRows.forEach(row => {
            ...
            const { data: badgeRows } = await supabase
                ...
                .eq("earned", true);
            const badgeMap = {};
            badgeRows === null || badgeRows === void 0 ? void 0 : badgeRows.forEach(row => {
                badgeMap[row.user_id] = (badgeMap[row.user_id] || 0) + 1;
            });
            // Merge and sort
            const leaderboard = Object.keys(xpMap).map(user_id => ({
                user_id,
                xp: xpMap[user_id],
    ...
    return (<div className="bg-white rounded-lg shadow p-4 mt-6">
      ...
      {loading ? (<div>Loading...</div>) : (<table className="min-w-full text-sm">
          <thead>
            <tr>
          ...
          <tbody>
            {leaders.map((entry, idx) => (<tr key={entry.user_id} className={idx === 0 ? "font-bold text-yellow-600" : ""}>
