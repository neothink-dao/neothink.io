import { Users, Heart, MessageSquare, Target, ArrowRight, Search, Award } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="flex items-center space-x-4 bg-white p-4 rounded-lg border border-gray-200">
          <div className="flex-1 flex items-center space-x-2 px-3 py-2 border rounded-md">
            ...
            <input type="text" placeholder="Search immortals, achievements, or discussions..." className="flex-1 outline-none bg-transparent"/>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {featuredMembers.map(member => (<div key={member.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <Link href={`/immortals/${member.id}`} className="text-emerald-600 hover:text-emerald-800 font-medium flex items-center">
            ...
            <div className="space-y-4">
              {projects.map(project => (<div key={project.id} className="p-4 border border-gray-100 rounded-lg">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Link href={`/projects/${project.id}`} className="text-emerald-600 hover:text-emerald-800 text-sm font-medium">
                      Join Project
                    </Link>
                  </div>
                </div>))}
        ...
        <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center space-x-4">
              <Link href="/discussions/new" className="bg-white text-emerald-600 px-6 py-3 rounded-lg border border-emerald-200 hover:border-emerald-300 transition-colors font-medium">
                Start Discussion
              </Link>
              <Link href="/projects/new" className="bg-emerald-600 text-white px-6 py-3 rounded-lg hover:bg-emerald-700 transition-colors font-medium">
                Create Project
              </Link>
            </div>
          </div>
