import { Target, Clock, LineChart, Calendar, CheckCircle, AlertCircle, Heart, Brain } from 'lucide-react';
    ...
    return (<div className="container mx-auto px-4 py-8">
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-6">
              {timeline.map(milestone => (<div key={milestone.id} className="relative">
                  <div className="flex items-start space-x-4">
                    <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center flex-shrink-0">
                      {milestone.completed ? (<CheckCircle className="text-emerald-600" size={20}/>) : (<Clock className="text-emerald-600" size={20}/>)}
                    </div>
                    <div className="flex-1">
                      ...
                      <div className="mt-3 flex items-center space-x-4">
                        ...
                        <Link href={`/milestones/${milestone.id}`} className="text-emerald-600 hover:text-emerald-800 text-sm font-medium">
                          View Details
                        </Link>
                      </div>
                    </div>
                  </div>
                  {milestone.id !== timeline.length && (<div className="absolute left-4 top-8 bottom-0 w-px bg-emerald-100"></div>)}
                </div>))}
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                {nextActions.map(action => (<div key={action.id} className="flex items-start space-x-3">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${action.priority === 'high' ? 'bg-red-100' : 'bg-emerald-100'}`}>
                      {action.priority === 'high' ? (<AlertCircle className="text-red-600" size={14}/>) : (<CheckCircle className="text-emerald-600" size={14}/>)}
                    </div>
                    <div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                {assessments.map(assessment => (<div key={assessment.id} className="p-3 border border-gray-100 rounded-lg">
                    <div className="flex items-center justify-between">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {objectives.map(objective => (<div key={objective.id} className="p-4 border border-gray-100 rounded-lg">
