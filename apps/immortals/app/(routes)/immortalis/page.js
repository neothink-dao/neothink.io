      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="flex items-center space-x-4 bg-white p-4 rounded-lg border border-gray-200">
          <div className="flex-1 flex items-center space-x-2 px-3 py-2 border rounded-md">
            ...
            <input type="text" placeholder="Search protocols, research, or topics..." className="flex-1 outline-none bg-transparent"/>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {categories.map(category => (<div key={category.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-2">
                {category.protocols.map(protocol => (<Link key={protocol.id} href={`/immortalis/${category.slug}/${protocol.slug}`} className="block p-3 rounded-lg hover:bg-gray-50 transition-colors">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              {research.map(item => (<Link key={item.id} href={`/research/${item.id}`} className="block p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition-colors">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              {breakthroughs.map(breakthrough => (<div key={breakthrough.id} className="p-4 border border-gray-100 rounded-lg">
                  ...
                  <Link href={`/breakthroughs/${breakthrough.id}`} className="text-emerald-600 hover:text-emerald-800 text-sm font-medium">
                    Learn More
                  </Link>
                </div>))}
            </div>
        ...
        <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <Link href="/immortalis/assessment" className="inline-flex items-center bg-emerald-600 text-white px-6 py-3 rounded-lg hover:bg-emerald-700 transition-colors font-medium">
              Start Assessment
            </Link>
          </div>
        </div>
