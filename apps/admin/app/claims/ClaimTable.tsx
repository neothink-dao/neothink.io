  ...
  return (
    <div>
      <div className="flex flex-wrap gap-4 mb-4">
        <select
          className="bg-slate-800 text-slate-200 rounded px-3 py-2"
          value={filterDay}
          onChange={(e) => setFilterDay(e.target.value)}
        >
          <option value="">All Days</option>
          {TOKEN_SCHEDULE.map((t) => (
            <option key={t.day} value={t.day}>{t.day}</option>
          ))}
        </select>
        <select
          className="bg-slate-800 text-slate-200 rounded px-3 py-2"
        ...
        >
          ...
          {TOKEN_SCHEDULE.map((t) => (
            <option key={t.token} value={t.token}>{t.token}</option>
          ))}
        </select>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full bg-slate-900 rounded-lg shadow-lg">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={5} className="text-center text-slate-400">Loading…</td></tr>
            ) : filteredClaims.length === 0 ? (
              <tr><td colSpan={5} className="text-center text-slate-400">No claims found.</td></tr>
            ) : (
              filteredClaims.map((c) => {
                ...
                return (
                  <tr key={c.id}>
                    ...
                    <td className={`px-4 py-2 font-bold bg-gradient-to-r ${tokenInfo?.gradient} text-transparent bg-clip-text`}>{c.token}</td>
