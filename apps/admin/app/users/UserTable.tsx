  ...
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full bg-slate-900 rounded-lg shadow-lg">
        ...
        <tbody>
          {loading ? (
            <tr><td colSpan={4} className="text-center text-slate-400">Loading…</td></tr>
          ) : users.length === 0 ? (
            <tr><td colSpan={4} className="text-center text-slate-400">No users found.</td></tr>
          ) : (
            users.map((u) => (
              <tr key={u.id}>
