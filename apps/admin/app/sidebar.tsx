...
export function Sidebar() {
  return (
    <aside className="fixed left-0 top-0 h-full w-64 bg-gradient-to-b from-zinc-900 via-slate-900 to-gray-950 shadow-xl z-40">
        ...
        <nav>
          {sections.map((section) => (
            <div key={section.label} className="mb-6">
              <div
                className={`text-lg font-semibold mb-2 bg-gradient-to-r ${section.gradient} text-transparent bg-clip-text`}
              >
                {section.label}
              </div>
              <ul>
                {section.routes.map((route) => (
                  <li key={route.path} className="mb-2">
                    <Link
                      href={route.path}
                      className="text-slate-200 hover:text-white transition-colors px-2 py-1 rounded block"
                    >
                      {route.label}
                    </Link>
                  </li>
