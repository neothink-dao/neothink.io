...
function getTokenGradient(token) {
    var _a;
    return ((_a = TOKEN_SCHEDULE.find((t) => t.token === token)) === null || _a === void 0 ? void 0 : _a.gradient) || '';
}
...
function BarChart({ data, label }) {
    ...
    return (<div className="mb-8">
      ...
      <div className="flex items-end h-32 gap-2">
        {data.map((d) => (<div key={d.label} className="flex flex-col items-center w-12">
            <div className={`w-10 rounded-t bg-gradient-to-b ${d.color}`} style={{ height: `${(d.value / max) * 100}%`, minHeight: 4 }} title={d.value.toString()}/>
    ...
    useEffect(() => {
        async function fetchAnalytics() {
            ...
            const { data: claims } = await supabase
                ...
                .select('token');
            const tokenCounts = {};
            claims === null || claims === void 0 ? void 0 : claims.forEach((c) => {
            ...
            users === null || users === void 0 ? void 0 : users.forEach((u) => {
                const week = new Date(u.created_at).toLocaleDateString('en-US', { year: 'numeric', week: 'numeric' });
