...
describe('Ascenders HomePage', () => {
  ...
  it('renders the page title', () => {
    render(<HomePage />);
    expect(screen.getByText('Ascenders Platform')).toBeInTheDocument();
  });

  it('tracks page view on initial render', () => {
    render(<HomePage />);
    expect(analytics.page).toHaveBeenCalledWith('ascenders', '/');
  });
  ...
  it('fetches and displays achievements', async () => {
    ...
    expect(mockEq).toHaveBeenCalledWith('platform', 'ascenders');
...
// Test API route separately in api.test.ts 