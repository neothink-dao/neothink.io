...
export default function AscensionPage() {
    ...
    return (<div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {MOCK_LESSONS.map((lesson) => (<QuestCard key={lesson.id} title={lesson.title} description={lesson.description} points={lesson.points} // Pass base points
         actionText={lesson.action} platform={lesson.platform} questType={lesson.type} // Cast for simplicity, ensure type safety
         isTrialUser={MOCK_USER.isTrial} onActionClick={() => console.log(`Action for ${lesson.id}`)} // Replace with actual action handler
        />))}
      </div>
