...
import { analyzeCode, findDocumentationFiles, findTestFiles, findFeatureFiles, findMigrationFiles, findRelatedFiles } from '../utils/code-analysis';
...
async function mapComponentContext(contextMap) {
...
async function mapDatabaseContext(contextMap) {
...
async function mapFeatureContext(contextMap) {
...
async function generateContextDocs(contextMap) {
...
async function generateAIMetadata(contextMap) {
...
async function findRelatedDocs(name) {
...
async function findRelatedTests(name) {
    return findTestFiles(name);
}
async function findRelatedFeatures(name) {
...
async function findDatabaseUsage(content) {
    const analysis = await analyzeCode(content);
    return {
...
async function findRelatedComponents(tableName) {
    const componentsDir = path.join(process.cwd(), 'components');
...
async function findRelatedMigrations(tableName) {
    return findMigrationFiles(tableName);
}
async function findFeatureComponents(featureName) {
...
async function findFeatureDatabaseUsage(featureName) {
    const featureDir = path.join(process.cwd(), 'features', featureName);
...
async function findFeatureTests(featureName) {
    const testPatterns = [
