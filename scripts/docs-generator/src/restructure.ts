...
const DOCUMENTATION_STRUCTURE: DocStructure = {
  'architecture': {
...
export async function restructureDocumentation(): Promise<void> {
  try {
...
async function organizeExistingDocs(): Promise<void> {
...
async function determineCategory(content: string): Promise<string | null> {
...
async function updateDocumentStructure(content: string, category: string): Promise<string> {
...
function addAIMetadata(content: string, category: string): string {
  const metadata = `---
category: ${category}
...
async function createIndexFiles(): Promise<void> {
