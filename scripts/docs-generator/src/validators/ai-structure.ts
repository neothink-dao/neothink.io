...
export async function validateAIFriendlyStructure(): Promise<void> {
  try {
    ...
    const validationResults: FileValidation[] = [];
...
async function validateFile(content: string): Promise<ValidationResult[]> {
  return validationRules.map(rule => rule.validate(content));
}

async function generateValidationReport(results: FileValidation[]): Promise<void> {
