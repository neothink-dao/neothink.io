...
const COMMENT_REQUIREMENTS: Record<string, CommentRequirements> = {
  component: {
 ...
 */
async function findRelatedFiles(content: string, file: string, line: number): Promise<string[]> {
...
export class AICommentsValidator {
    ...
    try {
      ...
      ast.comments?.forEach(comment => {
        if (comment.type === 'Block') {
          this.validateCommentBlock(comment.value, comment.loc?.start.line || 0, filePath);
        }
      });
   ...
   */
  private validateCommentBlock(comment: string, line: number, file: string) {
   ...
   */
  private async validateRelationships(content: string, file: string) {
    try {
   ...
   */
  private validateAIContext(comment: string, line: number, file: string) {
    const aiContextMatch = comment.match(/@ai-context\s*([\s\S]*?)(?=@|$)/);
    if (!aiContextMatch) return;
   ...
   */
  private getCommentType(comment: string): string | null {
    if (comment.includes('@component')) return 'component';
    if (comment.includes('@table')) return 'table';
    if (comment.includes('@feature')) return 'feature';
    if (comment.includes('@function')) return 'function';
    if (comment.includes('@type')) return 'type';
    return null;
  }
}
