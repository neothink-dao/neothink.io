...
interface LinkValidation {
  ...
  brokenLinks: {
    link: string;
    type: 'internal' | 'code-reference' | 'external';
    line: number;
  }[];
}

export async function validateDocLinks(): Promise<void> {
  try {
    logger.info('Validating documentation links...');

    const docsPath = path.join(process.cwd(), 'docs');
    const markdownFiles = await findMarkdownFiles(docsPath);
    const validationResults: LinkValidation[] = [];
...
async function validateFileLinks(filePath: string): Promise<LinkValidation> {
  const content = await fs.readFile(filePath, 'utf8');
  const lines = content.split('\n');
  const brokenLinks: LinkValidation['brokenLinks'] = [];
...
async function generateLinkReport(results: LinkValidation[]): Promise<void> {
...
function generateSuggestion(brokenLink: LinkValidation['brokenLinks'][0]): string {
